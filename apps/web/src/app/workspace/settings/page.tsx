...
function AiProvidersCatalog() {
  const providers = [
    ...
    },
    {
      ...
      slug: 'openrouter',
      description:
        'Agregador com acesso a centenas de modelos de diversos provedores. Todos os modelos disponíveis no OpenRouter estão listados abaixo.',
      models: [
        // Meta Llama
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-mute">
        Gerencie seu catálogo pessoal de modelos. Cada provedor mostra <strong>todos</strong> os
        modelos disponíveis — sem limite. Selecione os que deseja utilizar.
      </p>
      {providers.map((p) => (
          ...
          <div className="mt-2 px-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-1.5 max-h-80 overflow-y-auto">
              {p.models.map((m) => (
                <label
                  key={m}
                  className="flex items-center gap-2 text-sm py-1 px-2 rounded hover:bg-surfaceAlt cursor-pointer"
                >
  ...
  return (
    <div className="space-y-4 text-sm">
      <p className="text-mute">
        Os provedores de transcrição convertem áudio em texto. Cada um tem características e custos
        diferentes. Você pode escolher qual provedor utilizar para cada agente de transcrição.
      </p>
      ...
      <div className="space-y-3">
        <div className="p-4 rounded-lg bg-surfaceAlt">
          ...
          <div className="text-mute mt-1">
            <strong>Velocidade:</strong> Extremamente rápido (até 25x mais rápido que tempo real)
            graças à inferência em LPU.
          </div>
          <div className="text-mute mt-1">
            <strong>Custo:</strong> Gratuito no tier free (limites de rate). Para uso intensivo, US$
            0,111/hora de áudio.
          </div>
          <div className="text-mute mt-1">
            <strong>Qualidade:</strong> Excelente para português e inglês. Suporta detecção
        ...
        <div className="p-4 rounded-lg bg-surfaceAlt">
          ...
          <div className="text-mute mt-1">
            <strong>Velocidade:</strong> Rápido, mas mais lento que Groq. Tempo de processamento
            similar ao tempo real do áudio.
          </div>
          <div className="text-mute mt-1">
            <strong>Custo:</strong> US$ 0,006 por minuto de áudio (aproximadamente US$ 0,36/hora).
          </div>
          <div className="text-mute mt-1">
            <strong>Qualidade:</strong> Muito boa, especialmente para inglês. Suporta mais de 50
            idiomas.
          </div>
        </div>
        ...
        <div className="p-4 rounded-lg bg-surfaceAlt">
          ...
          <div className="text-mute mt-1">
            <strong>Velocidade:</strong> Depende do hardware. Com GPU NVIDIA, pode ser mais rápido
            que tempo real.
          </div>
          <div className="text-mute mt-1">
            <strong>Custo:</strong> Gratuito — roda localmente na sua infraestrutura. Requer GPU
            para melhor desempenho.
          </div>
          <div className="text-mute mt-1">
            <strong>Qualidade:</strong> Idêntica ao Whisper original. Suporta diarização
            (identificação de falantes).
          </div>
          <div className="text-mute mt-1">
            <strong>Requisitos:</strong> Docker + GPU NVIDIA (recomendado) ou CPU. Configurável via{' '}
            <code className="font-mono bg-bg px-1 rounded">LOCAL_WHISPER_URL</code>.
          </div>
        </div>
      </div>

      <p className="text-mute text-xs mt-4">
        💡 Recomendação: Use <strong>Groq</strong> para melhor custo-benefício. Use{' '}
        <strong>Faster Whisper local</strong> se preferir não enviar dados para a nuvem.
      </p>
    </div>
  );
}
