...
- Activation-audit scope now includes `transcription`, asserting runtime readiness via `OPENAI_API_KEY`, `GROQ_API_KEY`, or `LOCAL_WHISPER_URL` provider-path minimum.
- Deploy preflight contracts (`firebase-hosting.yml`, `release-platform.yml`) now emit explicit warning when no transcription path is configured and propagate optional `LOCAL_WHISPER_URL` into Cloud Run runtime env.
