...
- Transcription runtime now applies provider fallback in `packages/ai/src/providers/index.ts`: it attempts selected provider first, then configured alternatives (`groq`, `openai`, `elevenlabs`, `local-faster-whisper`), and preserves the explicit model only on the first provider attempt to avoid cross-provider model mismatches.
- Local Firestore emulator execution is now wrapped by `scripts/run-firestore-emulator-tests.mjs` with Java precheck, Windows Android Studio JBR fallback path detection, automatic busy-port rollover (default 8080 -> next free), and fixed emulator project pinning for stable test isolation.
- Root script `test:web:firestore-emulator` now routes through the wrapper and in this environment validates green with port rollover (`8080` busy -> `8081`) and full `emulator-routes` pass.
- Release closure snapshot (commit `cb15f31`, 2026-04-25 local): `CI` run `24942294925` success, `Pages` run `24942294927` success, `Firebase Hosting` run `24942294926` success.
- Firebase Hosting run `24942294926` completed in `7m23s`; deploy annotation flagged no configured transcription runtime path (`OPENAI_API_KEY`, `GROQ_API_KEY`, `ELEVENLABS_API_KEY`, `LOCAL_WHISPER_URL`) and remained non-blocking by current readiness policy.
...
- Activation-audit scope now includes `transcription`, asserting runtime readiness via `OPENAI_API_KEY`, `GROQ_API_KEY`, `ELEVENLABS_API_KEY`, or `LOCAL_WHISPER_URL` provider-path minimum.
- Deploy preflight contracts (`firebase-hosting.yml`, `release-platform.yml`) now emit explicit warning when no transcription path is configured (`OPENAI_API_KEY`, `GROQ_API_KEY`, `ELEVENLABS_API_KEY`, `LOCAL_WHISPER_URL`) and propagate optional `LOCAL_WHISPER_URL` into Cloud Run runtime env.
- Ops-readiness evidence orchestration script now exists at `scripts/run-ops-readiness-evidence.mjs`, dispatching workflow evidence chain (`Ops Activation Audit` targets + optional `Notifications Smoke` + optional `Audio Edit Runner`) and waiting for completion with consolidated JSON summary output.
- Root scripts now expose `pnpm ops:evidence:baseline` (non-strict activation targets) and `pnpm ops:evidence:strict` (strict full chain).
- Baseline orchestration evidence executed successfully (`pnpm ops:evidence:baseline -- --summary-path ops-readiness-evidence-baseline.json`) with activation run chain: notifications `24939698500`, audio-edit `24939703335`, transcription `24939707959`.
- Release closure snapshot (commit `b4ad704`, 2026-04-25 local): `CI` run `24939776631` success, `Pages` run `24939776635` success, `Firebase Hosting` run `24939776637` success.
- Transcription provider contract now includes ElevenLabs (`transcribeProvider=elevenlabs`, `AI_TRANSCRIBE_PROVIDER=elevenlabs`) with runtime key source `byokKeys.elevenlabs` / `ELEVENLABS_API_KEY` and model defaults `scribe_v2`/`scribe_v1`.
- Settings sanitization now preserves `byokKeys.elevenlabs`; worker + web run-task transcription calls now propagate ElevenLabs keys to `transcribe()`.
- Mobile quick-action path is now wired end-to-end: Android Quick Settings tile service is generated during prebuild (`plugins/quick-settings-tile`), emits deep links (`gravador://?quickAction=start|stop`), and `app/index.tsx` executes start/stop/toggle on the shared recorder flow.
- Lock-screen control now uses notification category `recording-controls` with action `toggle_recording`, routed in `app/_layout.tsx` to quick-action deep links.
- Mobile Expo config drift risk reduced by aligning `app.config.ts` defaults with runtime expectations (`firestoreDatabaseId=anotes`, notification color parity, `expo-localization` plugin parity).
- Local verification for transcription-readiness guardrail package is green: `pnpm lint:workflows`, `pnpm lint`, `pnpm typecheck`, `pnpm --filter @gravador/web run test`, `pnpm --filter @gravador/web run build`, `pnpm --filter @gravador/mobile run typecheck`, plus runtime checks `pnpm ops:audit:activation` and strict negative-path probe `node scripts/audit-ops-activation.mjs --strict --target transcription` (expected failure while provider path is unprovisioned).
- Release closure snapshot (commit `bf091fb`, 2026-04-25 local): `CI` run `24933107462` success, `Pages` run `24933107467` success, `Firebase Hosting` run `24933107458` success, manual `Ops Activation Audit` run `24933216975` success (`target=transcription`, `strict=false`).
