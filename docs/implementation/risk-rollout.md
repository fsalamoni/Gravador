...
- Expanded activation readiness audit with `target=transcription` and added deploy preflight warnings for missing transcription runtime path (`OPENAI_API_KEY`, `GROQ_API_KEY`, `LOCAL_WHISPER_URL`) in both `firebase-hosting.yml` and `release-platform.yml`.
...
- Transcription UX now minimizes misconfiguration risk and activation/deploy guardrails expose missing runtime path early, but strict transcription readiness still depends on provisioning valid BYOK secrets and/or a reachable `LOCAL_WHISPER_URL` endpoint.
