...
- Added centralized activation readiness audit path (`scripts/audit-ops-activation.mjs` + `.github/workflows/ops-activation-audit.yml`) with strict/non-strict scope checks, mitigating operator uncertainty around missing toggles/secrets before strict smoke/runner activation.
- Expanded activation readiness audit with `target=transcription` and added deploy preflight warnings for missing transcription runtime path (`OPENAI_API_KEY`, `GROQ_API_KEY`, `ELEVENLABS_API_KEY`, `LOCAL_WHISPER_URL`) in both `firebase-hosting.yml` and `release-platform.yml`.
- Added orchestration script `scripts/run-ops-readiness-evidence.mjs` + root commands `ops:evidence:baseline|strict` to reduce manual dispatch/watching errors while collecting strict operational evidence across audit/smoke/runner workflows.
- Expanded transcription runtime matrix with ElevenLabs support across schema/sanitization/provider runtime/UI (`transcribeProvider=elevenlabs`, `byokKeys.elevenlabs`, `ELEVENLABS_API_KEY`, `scribe_v2|scribe_v1`).
- Implemented Android quick capture controls via generated Quick Settings tile service (`QuickRecordTileService`) with deep-link quick actions (`start`/`stop`/`toggle`) handled in mobile home route.
- Added lock-screen notification action routing (`recording-controls` category + `toggle_recording`) so capture control can be triggered without navigating manually to the recorder screen.
- Reduced APK startup drift by aligning `app.config.ts` defaults with stable mobile runtime assumptions (`firestoreDatabaseId=anotes`, notification color parity, localization plugin parity).
- Local validation after operational activation audit package remained green (`pnpm lint`, `pnpm typecheck`, `pnpm --filter @gravador/web run test`, `pnpm --filter @gravador/web run build`, `pnpm --filter @gravador/mobile run typecheck`, plus `pnpm ops:audit:activation` runtime sanity execution).
- Release verification for commit `834e8ef`: `CI` run `24928487774` success, `Pages` run `24928487777` success, `Firebase Hosting` run `24928487778` success, manual `Firestore Managed E2E` run `24928509646` success (`database_id=anotes`).
- Initial manual `Ops Activation Audit` run `24928511832` failed in summary publication due shell heredoc delimiter parsing; first fix commit `e8aafa6` then exposed malformed workflow parsing on push run `24928546801` (no jobs executed), confirming indentation fragility.
- Mitigation finalized on commit `4bb9ce6`: summary publication now uses single-command `node -e` output writer, removing heredoc delimiter/indentation risk.
- Additional prevention package added: CI now runs `pnpm lint:workflows` through a dedicated `workflow-lint` job (`yaml-lint` over `.github/workflows/*.yml`) to catch malformed workflow YAML before merge.
- Local validation after workflow-lint guardrail package remained green (`pnpm lint`, `pnpm lint:workflows`, `pnpm typecheck`, `pnpm --filter @gravador/web run test`, `pnpm --filter @gravador/web run build`, `pnpm --filter @gravador/mobile run typecheck`).
- Local validation after transcription-readiness guardrail package remained green (`pnpm lint:workflows`, `pnpm lint`, `pnpm typecheck`, `pnpm --filter @gravador/web run test`, `pnpm --filter @gravador/web run build`, `pnpm --filter @gravador/mobile run typecheck`, `pnpm ops:audit:activation`, and strict negative-path probe `node scripts/audit-ops-activation.mjs --strict --target transcription` confirming deterministic failure when no provider path is configured).
- Local validation after ops-readiness evidence orchestration package remained green (`pnpm lint`, `pnpm typecheck`, `pnpm --filter @gravador/web run test`, `pnpm --filter @gravador/web run build`, `pnpm --filter @gravador/mobile run typecheck`, plus orchestration smoke run `pnpm ops:evidence:baseline`).
- Baseline orchestration evidence chain executed with all-success outcomes: `Ops Activation Audit` notifications run `24939698500`, audio-edit run `24939703335`, transcription run `24939707959`.
- Release verification for commit `b4ad704`: `CI` run `24939776631` success, `Pages` run `24939776635` success, `Firebase Hosting` run `24939776637` success.
- Release verification for commit `bf091fb`: `CI` run `24933107462` success, `Pages` run `24933107467` success, `Firebase Hosting` run `24933107458` success, manual `Ops Activation Audit` run `24933216975` success (`target=transcription`, `strict=false`).
- Final verification for commit `4bb9ce6`: `CI` run `24928579016` success, manual `Ops Activation Audit` run `24928594813` success, manual `Firestore Managed E2E` run `24928596802` success (`database_id=anotes`), manual `Firebase Hosting` run `24928598960` success, manual `Pages` run `24928600630` success.
- Release verification for commit `6aebdb8`: `CI` run `24932359275` success, `Pages` run `24932359268` success, `Firebase Hosting` run `24932359265` success, manual `Firestore Managed E2E` run `24932364466` success (`database_id=anotes`), manual `Ops Activation Audit` run `24932365170` success.
